...
export function StartScreen({ eraGroups, onStart }: StartScreenProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-6 p-4 pt-16 text-center md:max-w-3xl">
      ...
      <Card className="gap-0 py-0 text-left md:flex-1">
        <div className="flex min-h-13 flex-wrap items-center justify-between gap-x-3 gap-y-1 rounded-t-xl border-b px-(--card-spacing) py-3">
      ...
      <Card className="gap-0 py-0 text-left md:flex-1">
        <div className="flex min-h-13 flex-wrap items-center justify-between gap-x-3 gap-y-1 rounded-t-xl border-b px-(--card-spacing) py-3">
          ...
          <div className="flex shrink-0">
            <Button variant="ghost" size="sm" onClick={selectAllSets}>
              Select all
            </Button>
            <Button variant="ghost" size="sm" onClick={deselectAllSets}>
              Deselect all
            </Button>
          </div>
        </div>
        ...
        <CardContent className="max-h-80 overflow-y-auto p-0">
          <div className="flex flex-col gap-4 px-(--card-spacing) pt-3 pb-(--card-spacing)">
            {eraGroups.map((group) => {
                  ...
                  <FieldLegend variant="label">
                    <Field orientation="horizontal">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          toggleEra(group, checked === true)
                        }
                      />
                      <FieldLabel htmlFor={`era-${group.era}`}>
                        {group.era}
                      </FieldLabel>
                    </Field>
                  </FieldLegend>
